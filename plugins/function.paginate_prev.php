...
function smarty_function_paginate_prev($params, &$smarty) {
    ...
    if(($_item = $sp->_getPrevPageItem($_id)) !== false) {
        $_show = true;
        $_text = isset($params['text']) ? $params['text'] : $sp->getPrevText($_id);
        $_url .= (strpos($_url, '?') === false) ? '?' : '&';
        $_url .= $sp->getUrlVar($_id) . '=' . $_item;
    } else {
        $_show = false;   
    }
